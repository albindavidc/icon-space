...
function fixBadgeMap(map) {
  for (const [key, val] of Object.entries(map)) {
    if (overrides[key]) {
      val.bg = overrides[key];
    } else if (val.bg === '000000' && !['vercel', 'notion', 'github', 'apple', 'actix', 'nextjs', 'next'].includes(key)) {
      // some defaults were black that shouldn't be, but it's hard to guess without overriding explicitly.
    }
    
    // recalculate fg
    const hex = val.bg.replace('#', '');
